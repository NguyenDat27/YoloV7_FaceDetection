...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            t = time_synchronized()
            out = non_max_suppression(out, conf_thres, iou_thres, agnostic=single_cls, multi_label=True, labels=lb)
        ...
        for si, pred in enumerate(out):
            ...
            if nl:
                ...
                scale_coords(img[si].shape[1:], tbox, shapes[si][0], shapes[si][
                    1])  # native-space labels, kpt_label is set to False as we are dealing with boxes here
                if kpt_label:
                    tkpt = labels[:, 5:]
                    scale_coords(img[si].shape[1:], tkpt, shapes[si][0], shapes[si][1])  # native-space labels
